...
散列表，有这么一个函数可以在0~Table-1均匀的分配关键字。主要问题：选择一个可以这样做的函数，解决冲突（当两个关键字被散列到同一个值时）以及如何确定散列表的大小。
...
$$
f(x)=(a_{n}x^{n-1}+a_{n-1}x^{n-2}+a_{n-2}x^{n-3}+......+a_{2}x+a_{1})x+a_{0}
$$

$$
 f(x)=((a_{n}x^{n-2}+a_{n-1}x^{n-3}+a_{n-2}x^{n-4}+......+a_{2})x+a_{1})x+a_{0}
$$

$$
f(x)=(((a_{n}x+a_{n-1})x+a_{n-2})x+......+a_{1})x+a_{0}
$$
...
**结构性：**底层从左到右填入，是一颗被完全填满的二叉树。这样的树称为完全二叉树(complete binary tree)。

**堆序性：** 能够快速找到最小元。因此最小元在根上，将任意子树也看成堆，则其儿子节点中元素值一定比该节点中元素值大或者相等。

**堆操作：** 插入、删除最小元

​	插入(Insert)：策略：**上滤**(percolate up)

​	删除最小元(DeleteMin)：策略：**下滤**(percolate down)
...
​	**BuildHeap(构建堆)**  -- **证明**其运行时间为O(N) N为节点数